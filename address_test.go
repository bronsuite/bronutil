...
func TestAddresses(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			f: func() (bronutil.Address, error) {
				pkHash := []byte{
					0x75, 0x1e, 0x76, 0xe8, 0x19, 0x91, 0x96, 0xd4, 0x54, 0x94,
					0x1c, 0x45, 0xd1, 0xb3, 0xa3, 0x23, 0xf1, 0x43, 0x3b, 0xd6}
				return bronutil.NewAddressWitnessPubKeyHash(pkHash, &customParams)
			},
			net: &customParams,
		},
		...
		// P2TR address tests.
		{
			...
			valid:   true,
			result: bronutil.TstAddressTaproot(
				1, [32]byte{
			...
			f: func() (bronutil.Address, error) {
				...
				return bronutil.NewAddressTaproot(
	...
	for _, test := range tests {
		...
		if err == nil {
			...
			case *bronutil.AddressTaproot:
				saddr = bronutil.TstAddressTaprootSAddr(encoded)
			}
